...
export default function MediaGallery({
  ...
  const [sortBy, setSortBy] = useState<
    "createdAt" | "size" | "originalName" | "usageCount"
  >("createdAt");
  ...
  const handleDelete = async (fileIds: string[]) => {
    if (
      !confirm(
        `Are you sure you want to delete ${fileIds.length} file(s)? This action cannot be undone.`,
      )
    ) {
      return;
    }
    ...
    try {
      if (fileIds.length === 1) {
        ...
        if (response.success) {
          toast.success(
            `${response.data.deleted.length} files deleted successfully`,
          );
          if (response.data.failed.length > 0) {
            toast.error(
              `${response.data.failed.length} files failed to delete`,
            );
          }
        }
      }
          ...
          <div className="flex flex-wrap items-center gap-4">
            ...
            <select
              ...
              onChange={(e) => {
                ...
                setSortBy(
                  field as "createdAt" | "size" | "originalName" | "usageCount",
                );
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
              {filteredFiles.map((file) => (
                <div
                  key={file._id}
                  className={`relative group bg-white border-2 rounded-lg overflow-hidden cursor-pointer transition-all ${
                    selectedFiles.has(file._id)
                      ? "border-indigo-500 ring-2 ring-indigo-200"
                      : "border-gray-200 hover:border-gray-300"
                  } ${selectionMode ? "hover:ring-2 hover:ring-indigo-200" : ""}`}
          ...
          <div className="bg-white rounded-lg max-w-4xl max-h-[90vh] overflow-auto">
            ...
            <div className="p-4">
              ...
              <div className="mt-4 space-y-2 text-sm text-gray-600">
                <p>
                  <strong>Size:</strong> {previewFile.formattedSize}
                </p>
                <p>
                  <strong>Type:</strong> {previewFile.type}
                </p>
                <p>
                  <strong>Category:</strong> {previewFile.category}
                </p>
                {previewFile.alt && (
                  <p>
                    <strong>Alt text:</strong> {previewFile.alt}
                  </p>
                )}
                {previewFile.caption && (
                  <p>
                    <strong>Caption:</strong> {previewFile.caption}
                  </p>
                )}
